...
class AttentionQKV(nn.Module):
    ...
    def forward(self, queries, keys, values, mask=None):
        ...
        if len(keys.shape)==3:
            similarity = queries.matmul(keys.permute((0,2,1))/th.sqrt(key_dim)) # Compute the similarity according to the QKV formula
        if len(keys.shape)==4:
            similarity = queries.matmul(keys.permute((0,1,3,2)))/th.sqrt(key_dim) 
        ...
        if len(keys.shape)==4:
            weights = nn.Softmax(dim=3)(masked_similarity)
...
class MultiHeadProjection(nn.Module):
    ...
    def _split_heads(self, tensor):
        ...
        #tch_size, n_heads, sequence_length, new_feature_size
        ...
        #tensor=tensor.reshape((batch_size, self.n_heads, tensorlen,int(new_feature_size)))
    ...
    def _combine_heads(self, tensor):
        ...
        assert tensor.shape[2]==self.n_heads
        new_feature_size = feature_size*self.n_heads  # What is the new feature size, if we combine all the heads
        
        tensor =  tensor.reshape((batch_size,tensorlen,int(new_feature_size)))# Reshape the Tensor to remove the heads dimension and come back to a Rank-3 tensor
        return tensor
        ##########################################################################################
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, inputs, mask=None):
        ...
        """
        assert (isinstance(inputs, tuple) or isinstance(inputs, list) or th.is_tensor(inputs) )  and len(inputs) == 2 and 'Must pass query and memory'
