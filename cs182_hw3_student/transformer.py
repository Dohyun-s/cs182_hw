...
class PositionEmbedding(nn.Module):
    ...
    def forward(self, inputs, start=1):
        ...
        return  pos_embed_deviced+inputs# add the embedding to the input
        ####################################  END OF YOUR CODE  ##################################
...
class TransformerFeedForward(nn.Module):
    ...
    def forward(self, inputs):
        ...
        dense_out =self.feed_forward(norm_input)+inputs
        dense_drop = self.dropout(dense_out) # Add the dropout here
        return dense_drop  # Add the residual here
        ####################################  END OF YOUR CODE  ##################################
...
class TransformerDecoderBlock(nn.Module):
    ...
    def forward(self, decoder_inputs, encoder_outputs, self_attention_mask=None, cross_attention_mask=None):
        ...
        norm_target_selfattn =self.cross_attention((self.cross_norm_target(res_target_self_attn),self.cross_norm_source(encoder_outputs)), mask=cross_attention_mask)
        norm_encoder_outputs =norm_target_selfattn+res_target_self_attn
      #  encdec_attention =norm_encoder_outputs+
       ...
       # res_encdec_attention =encdec_attention+res_target_self_attn
        ...
        output =self.feed_forward(norm_encoder_outputs)
...
class Transformer(nn.Module):
    ...
    def forward(self, source_sequence, target_sequence, encoder_mask, decoder_mask, mask_future=True, shift_target_sequence_right=True):
        ...
        encoder_output =self.encoder.forward(source_sequence, encoder_mask=encoder_mask)
        ...
        decoder_output =self.decoder.forward(target_input=target_sequence,
